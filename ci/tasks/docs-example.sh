...
export GH_REPO=cafeshadown/${repo}
...
#install terraform
curl -OL https://releases.hashicorp.com/terraform/1.5.5/terraform_1.5.5_linux_amd64.zip
unzip -o terraform_1.5.5_linux_amd64.zip -d /usr/local/bin
    ...
    if [[ ${fileName} == "alicloud/resource_alicloud"* || ${fileName} == "alicloud/data_source_alicloud"* || ${fileName} == "website/docs/r/"* ]]; then
      ...
      cat ${docsDir} | while read line; do
        ...
        if [[ $line == '```' && "${begin}" = "true" ]]; then
          ...
          if [ $? -ne 0 ]; then
            if [[ ${exampleTerraformErrorTmpLog} == *"SDKError"* ]]; then
          ...
          #   terraform destroy
          { terraform -chdir=${exampleFileName} plan -destroy && terraform -chdir=${exampleFileName} apply -destroy -auto-approve; } 2>${exampleTerraformErrorTmpLog} >>${exampleTestRunLog}
