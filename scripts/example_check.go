...
func main() {
	...
	runLogFileName := "terraform-example.run.log"
	...
	for !time.Now().After(deadline) {
		...
		if runLogResponse.StatusCode == 200 {
			if lineNum > lastLineNum {
				...
				log.Println(string(runLogContent[lastLineNum:lineNum]))
				lastLineNum = lineNum
			}
		}
		if finish {
		...
		runResultResponse, err := http.Get(runLogUrl)
		...
		finish = true
		if strings.Contains(string(runResultContent), "Fail") {
			exitCode = 1
		}
	}
